package issparser.teamnames;

import com.google.common.io.ByteSource;
import com.google.common.io.Files;
import issparser.commons.ParsingUtils;
...
public class TeamNameRomHandler implements RomHandler<String> {
    ...
    public void writeToRom(Iterable<String> input) throws IOException {
        ...
        for (long teamOffset : TEAM_OFFSETS) {
            ...
            for (int i=0; i<relevantBytes.length; i++) {
                //issparser.commons.FileUtils.writeToPosition(rom, new byte[]{relevantBytes[relevantBytes.length-1-i]}, teamOffset + (4 * i));
            }
        }
    ...
    public static String teamNameText(byte[] bytes) {
        ...
        for (int i=0; i<bytes.length; i++) {
            ...
            if(i%2 == 0) {
                //System.out.println(issparser.commons.ParsingUtils.bytesString(new byte[]{bytes[i]}));
